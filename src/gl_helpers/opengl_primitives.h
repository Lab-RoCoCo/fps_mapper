...
namespace GLHelpers {
  ...
  /**
   * multiplies to the right an Eigen isometry (or compatible, e.g. Eigen::Affine3f) to
   * the top element of the gl stack. this function expects a 4x4 float matrix (not
   * double)!
   * 
   * explicit template types (Eigen::Isometry3f and Eigen::Affine3f) are provided (
   * required for using this in a shared library)
   */
  template<typename T> void glMultMatrix(const T& iso);
